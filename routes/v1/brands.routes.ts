 ...
 *     summary: Retrieve a list of products that are assigned to a type
 *     description: Retrieve a list of products assigned to a given product type.
 *     parameters:
 *       - in: path
 *         name: id
 *         required: true
 ...
 *          description: Internal server error
 */
brandRouter.get("/:id/products", async (req, res) => {
  const { id } = req.params;
  if (!Number.isNaN(Number(id))) {
    try {
      const products = await getProductsByBrand(Number(id));
